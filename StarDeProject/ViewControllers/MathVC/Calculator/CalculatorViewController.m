...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        [self setNavigation:@"计算器" hasBackBtn:YES];
    }
    return self;
}

- (void)setNavigation:(NSString *)title hasBackBtn:(BOOL)hasBackBtn
{
    UIView *headerView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 64)];
    headerView.backgroundColor = [UIColor colorWithRed:245/255.0 green:80/255.0 blue:83/255.0 alpha:1];
    ...
    if (hasBackBtn)
    {
        UIButton *backBn =  [[UIButton alloc]initWithFrame:CGRectMake(10, 27, 31, 31)];
        [backBn setImage:[UIImage imageNamed:@"back"] forState:UIControlStateNormal];
        [backBn addTarget:self action:@selector(backToLastVC) forControlEvents:UIControlEventTouchUpInside];
        [headerView addSubview:backBn];
    }
}
- (void)backToLastVC
{
